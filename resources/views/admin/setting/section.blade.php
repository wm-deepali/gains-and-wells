...
<td>{{ $setting->page_slug }}</td>
<td>{{ ucfirst($setting->is_show_faq_form) ?? "" }}</td>
<td>{{ ucfirst($setting->faq_type) ?? "" }}</td>
<td>{{ ucfirst($setting->is_show_logo_marque) ?? "" }}</td>
<td>{{ ucfirst($setting->is_show_blog_section) ?? "" }}</td>
<td>{{ ucfirst($setting->is_show_testimonial) ?? "" }}</td>
<td class="text-truncate">
    <a href="javascript:void(0)" class="edit-section text-dark mr-2" page_id="{{ $setting->id }}" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>
    <a href="javascript:void(0)" class="text-danger" onclick="deleteConfirmation({{ $setting->id }})" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></a>
</td>
</tr>
@endforeach
@endif
...
<script>
    ...
    function deleteConfirmation(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ URL::to('admin/manage-sections/${id}') }}`,
                    type: "DELETE",
                    dataType: "json",
    ...
    $(document).ready(function() {
        $(document).on("click", ".add-section", function(event) {
            $.ajax({
                url: "{{ URL::to('admin/manage-sections/create') }}",
                type: "GET",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
        ...
        $(document).on("click", ".edit-section", function(event) {
            ...
            $.ajax({
                url: `{{ URL::to('admin/manage-sections/${id}/edit') }}`,
                type: "get",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
