...
<tbody> @php $i=1; @endphp
        ...
        <tr>
            ...
            <td class="text-truncate">
                <a href="javascript:void(0)" class="edit-state text-dark mr-2" state_id="{{ $state->id }}" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                <a href="javascript:void(0)" class="text-danger" onclick="deleteConfirmation({{ $state->id }})" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></a>
            </td>
        </tr>
        @endforeach
        @endif
...
<script>
    function deleteConfirmation(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ URL::to('admin/manage-state/${id}') }}`,
                    type: "DELETE",
                    dataType: "json",
    ...
    $(document).ready(function() {
        $(document).on("click", ".add-state", function(event) {
            $.ajax({
                url: "{{ URL::to('admin/manage-state/create') }}",
                type: "GET",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
        ...
        $(document).on("click", ".edit-state", function(event) {
            ...
            $.ajax({
                url: `{{ URL::to('admin/manage-state/${id}/edit') }}`,
                type: "get",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
